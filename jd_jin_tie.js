							...
							if (info) {
								...
								if ($.recevieTask && $.recevieTask.length) {
									for (let task of $.recevieTask) {
										console.log('预计获得：', task.awards[0].awardName, task.awards[
											0].awardRealNum, task.awards[0].awardUnit)
				...
				if (err) {
					...
					if (data.resultCode === 0) {
						if (data.resultData.code === '000') {
							...
							console.log('获取签到状态成功', state ? '今日已签到' : '今日未签到', '连续签到', data
								.resultData.data.signContinuity, '天\n')
...
async function doTask() {
	for (let task of $.willTask) {
		...
		} else if ((task.detail.indexOf('关注') !== -1 || task.detail.indexOf('收藏')) && task.doLink.indexOf(
			'shopId') !== -1) {
