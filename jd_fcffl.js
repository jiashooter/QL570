			...
			if (cookie) {
				...
				if (leftTime != 0) {
					...
					if ($.canDraw) {
						...
						await Draw($.reward.id, $.reward.poolBaseId, $.reward.prizeGroupId, $.reward
							.prizeBaseId, $.reward.prizeType)
						await totalPrize()
						...
						for (item in data.data.items) {
							...
							if (reward.prizeType === 4) {
								...
								if (reward.state === 0) {
									...
									await Draw(reward.id, reward.poolBaseId, reward
										.prizeGroupId, reward.prizeBaseId, reward.prizeType)
									await $.wait(500);
								}
					...
					if (data.code === 0 && data.data) {
						if (functionid != "gambleObtainReward") {
							...
							if (data.data.rewardState === 3) {
								...
								console.log("翻倍成功啦")
								console.log("当前红包：" + data.data.rewardValue + "翻倍次数：" + data
									.data.changeTimes)
							} else {
...
function Draw(id, poolBaseId, prizeGroupId, prizeBaseId, prizeType) {
	return new Promise(async (resolve) => {
		let options = taskPostUrl("apCashWithDraw",
