...
message = ""!(async () => {
		...
		$.shareUuid = '3aba0191d18741eab463b645d3a7cd59'
		console.log(
		...
		for (let i = 0; i < cookiesArr.length; i++) {
			...
			if (cookie) {
				...
				await getUserInfo();
				$.attrTouXiang =
					'http://storage.360buyimg.com/i.imageUpload/6a645f3437633463333562316434363231353937323838313433353232_mid.jpg'
				$.actorUuid = '';
				$.actorUuid = await getActorUuid();
				console.log("邀请码：" + $.actorUuid)
				if (!$.actorUuid) {
				...
				if (checkOpenCardData && checkOpenCardData.score == 1) await startDraw(1)
				$.log("关注: " + $.followShop)
				if (!$.followShop) await followShop();
				if (!$.followShop) await $.wait(1000)
				$.log("加购: " + $.addSku)
				if (!$.addSku) await addSku();
...
function getDrawRecordHasCoupon() {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}`
		$.post(taskPostUrl('/dingzhi/taskact/openCardcommon/getDrawRecordHasCoupon', body), async (err, resp,
			data) => {
			try {
				if (err) {
					...
					for (let i in data.data) {
						...
						if (item.drawId != '邀请好友') console.log(
							`${item.infoType != 10 && item.value +':' || ''}${item.infoName}`)
					}
...
function getShareRecord() {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&num=0&sortSuatus=1`
...
function addSku() {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=2&taskValue=100022672084`
...
function followShop() {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&taskType=1&taskValue=1&shareUuid=${$.shareUuid}`
		...
		$.get(shopactivityId(`${venderId}`), async (err, resp, data) => {
			try {
				...
				if (data.success == true) {
					...
					$.shopactivityId = data.result.interestsRuleList && data.result
						.interestsRuleList[0] && data.result.interestsRuleList[0].interestsInfo &&
...
function shopactivityId(functionId) {
	return {
		...
		headers: {
			...
			'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=33d9ae00c1904c20bd8a4488fc5d6f33&shareUuid=${$.shareUuid}`,
			'Cookie': cookie
		}
	}
}
...
function startDraw(type) {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.myPingData.secretPin)}&type=${type}`
		...
		$.post(taskPostUrl('/dingzhi/dz/openCard/startDraw', body), async (err, resp, data) => {
			try {
				if (err) {
					...
					// $.log($.toStr(data))
					data = $.toObj(data);
					if (data.errorMessage || data.data.errorMessage) console.log(
						`抽奖：${data.errorMessage || data.data.errorMessage || ''}`)
					if (data.count == 0 && data.result == true) {
...
function checkOpenCard() {
	return new Promise(resolve => {
		let body =
			`activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pin=${encodeURIComponent($.myPingData.secretPin)}&actorUuid=${$.actorUuid}&shareUuid=${$.shareUuid}`
...
function getMyPing() {
	...
	return new Promise(resolve => {
		$.post({
			...
			body: `userId=1000016162&token=${$.isvObfuscatorToken}&fromType=APP`,
			headers: {
				'User-Agent': $.UA,
				'Content-Type': 'application/x-www-form-urlencoded',
				'Host': 'lzdz1-isv.isvjcloud.com',
			...
			try {
				if (err) {
					...
					data = JSON.parse(data);
					let setcookie = resp['headers']['set-cookie'] || resp['headers'][
						'Set-Cookie'] || ''
					if (setcookie) {
						let lz_jdpin_token = setcookie.filter(row => row.indexOf(
							"lz_jdpin_token") !== -1)[0]
						$.lz_jdpin_token = ''
						if (lz_jdpin_token && lz_jdpin_token.indexOf("lz_jdpin_token=") > -1) {
							$.lz_jdpin_token = lz_jdpin_token.split(';') && (lz_jdpin_token.split(
								';')[0] + ';') || ''
						}
						let LZ_TOKEN_VALUE = setcookie.filter(row => row.indexOf(
							"LZ_TOKEN_VALUE") !== -1)[0]
						if (LZ_TOKEN_VALUE && LZ_TOKEN_VALUE.indexOf("LZ_TOKEN_VALUE=") > -1) {
							$.LZ_TOKEN_VALUE = LZ_TOKEN_VALUE.split(';') && (LZ_TOKEN_VALUE.split(
								';')[0]) || ''
...
function getHtml() {
	...
	return new Promise(resolve => {
		$.get({
			url: `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=33d9ae00c1904c20bd8a4488fc5d6f33&shareUuid=${$.shareUuid}`,
			headers: {
				'User-Agent': $.UA,
				'Host': 'lzdz1-isv.isvjcloud.com',
...
function adLog() {
	return new Promise(resolve => {
		...
		$.post({
			...
			body: `venderId=1000016162&code=99&pin=${encodeURIComponent($.myPingData.secretPin)}&activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pageUrl=${encodeURIComponent(pageurl)}&subType=APP&adSource=null`,
			headers: {
				'User-Agent': $.UA,
				'Host': 'lzdz1-isv.isvjcloud.com',
			...
			try {
				if (err) {
					...
					let setcookie = resp['headers']['set-cookie'] || resp['headers'][
						'Set-Cookie'] || ''
					if (setcookie) {
						let LZ_TOKEN_KEY = setcookie.filter(row => row.indexOf("LZ_TOKEN_KEY") !== -
							1)[0]
						if (LZ_TOKEN_KEY && LZ_TOKEN_KEY.indexOf("LZ_TOKEN_KEY=") > -1) {
							$.LZ_TOKEN_KEY = LZ_TOKEN_KEY.split(';') && (LZ_TOKEN_KEY.split(';')[
								0]) || ''
...
function drawContent() {
	return new Promise(resolve => {
		$.post({
			...
			body: `activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pin=${encodeURIComponent($.myPingData.secretPin)}`,
			headers: {
				'User-Agent': $.UA,
				'Host': 'lzdz1-isv.isvjcloud.com',
				'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
				'Referer': `https://lzdz1-isv.isvjcloud.com/dingzhi/dz/openCard/activity/832865?activityId=33d9ae00c1904c20bd8a4488fc5d6f33&shareUuid=${$.shareUuid}`,
				'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE}; AUTH_C_USER=${$.myPingData.secretPin}; ${$.lz_jdpin_token}`,
			}
		}, async (err, resp, data) => {
			try {
				...
				if (err) {
					...
					if (data.count === 0 && data.result) {
						$.attrTouXiang = data.data.yunMidImageUrl !=
							data.data.yunMidImageUrl ? $.attrTouXiang = data.data.yunMidImageUrl : $
							.attrTouXiang =
...
function getActorUuid() {
	return new Promise(resolve => {
		$.post({
			...
			body: `activityId=33d9ae00c1904c20bd8a4488fc5d6f33&pin=${encodeURIComponent($.myPingData.secretPin)}&pinImg=${encodeURIComponent($.attrTouXiang)}&nick=${encodeURIComponent($.myPingData.nickname)}&cjyxPin=&cjhyPin=&shareUuid=${$.shareUuid}`,
			headers: {
				'User-Agent': $.UA,
				'Host': 'lzdz1-isv.isvjcloud.com',
				'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
...
function getUA() {
	$.UA =
		`jdapp;iPhone;10.0.10;14.3;${randomString(40)};network/wifi;model/iPhone12,1;addressid/4199175193;appBuild/167741;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1`
}
