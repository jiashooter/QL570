...
let cookiesArr = [];
const activityList = [{
		'actid': '4d2f7df45a0e4a1b8d663e7da0fc0d0d',
		'endTime': 1628394029000
	},
	{
		'actid': '4776be60946e45b1847bd982e24b4aa9',
		'endTime': 1628438400000
	},
	{
		'actid': 'c75ae2afd7ff4aec9ed47008b08400f7',
		'endTime': 1630288800000
	},
	{
		'actid': 'ea52a4da34d34be0a1c6470bd7d92063',
		'endTime': 1628352000000
	},
	{
		'actid': '3da50af9e8664746844c5456b8920b7d',
		'endTime': 1630425599000
	},
	{
		'actid': '4374884673374f9c883d21ceea5694f1',
		'endTime': 1628524799000
	},
	{
	...
	{
		'actid': '31073025b8a34de59d8d55faffdd44ab',
		'endTime': 1630425599000
	},
	{
		'actid': '5a8aea7f27b84900a14624fe9dcc8fe1',
		'endTime': 1628956799000
	},
	{
		'actid': 'f61f162f3b9d4e3eb457f2193bf12b80',
		'endTime': 1628611199000
	},
...
async function main() {
	...
	let endtime = date.getFullYear() + "-" + (date.getMonth() < 10 ? '0' + (date.getMonth() + 1) : (date
	.getMonth() + 1)) + "-" + (date.getDate() < 10 ? '0' + date.getDate() : date.getDate())
	console.log(
		`${$.actinfo.actName},${$.actinfo.shopName},当前积分：${$.nowUseValue},结束时间：${endtime}，${$.activityData.actInfo.endTime}`
		);
...
async function luckDraw() {
	...
	if (nowUseValue >= $.activityData.actConfig.prizeScoreOne && $.activityData.actorInfo.prizeOneStatus ===
		false) {
		console.log(`开始第一次抽奖`);
		$.drawType = '01';
	...
	if (nowUseValue >= $.activityData.actConfig.prizeScoreTwo && $.activityData.actorInfo.prizeTwoStatus ===
		false) {
		console.log(`开始第二次抽奖`);
		$.drawType = '02';
		await takePostRequest('startDraw');
	...
	if (nowUseValue >= $.activityData.actConfig.prizeScoreThree && $.activityData.actorInfo.prizeThreeStatus ===
		false) {
		console.log(`开始第三次抽奖`);
		$.drawType = '03';
		await takePostRequest('startDraw');
...
async function doTask() {
	...
	if ($.activityData.task2BrowGoods && $.runFalag) {
		if ($.activityData.task2BrowGoods.finishedCount !== $.activityData.task2BrowGoods.upLimit) {
			needFinishNumber = Number($.activityData.task2BrowGoods.upLimit) - Number($.activityData.task2BrowGoods
				.finishedCount);
			console.log(`开始做浏览商品任务`);
			$.upFlag = true;
			for (let i = 0; i < $.activityData.task2BrowGoods.taskGoodList.length && needFinishNumber > 0 && $
				.runFalag; i++) {
	...
	if ($.activityData.task4Share && $.runFalag) {
		if ($.activityData.task4Share.finishedCount !== $.activityData.task4Share.upLimit) {
			needFinishNumber = Number($.activityData.task4Share.upLimit) - Number($.activityData.task4Share
				.finishedCount);
			console.log(`开始做分享任务`);
			$.upFlag = true;
	...
	if ($.activityData.task6GetCoupon && $.runFalag) {
		if ($.activityData.task6GetCoupon.finishedCount !== $.activityData.task6GetCoupon.upLimit) {
			needFinishNumber = Number($.activityData.task6GetCoupon.upLimit) - Number($.activityData.task6GetCoupon
				.finishedCount);
			console.log(`开始做领取优惠券`);
			$.upFlag = true;
			for (let i = 0; i < $.activityData.task6GetCoupon.taskCouponInfoList.length && needFinishNumber > 0 && $
				.runFalag; i++) {
...
async function takePostRequest(type) {
	...
	switch (type) {
		...
		case 'doGetCouponTask':
			url = `https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/${type}`;
			body =
				`activityId=${$.activityID}&uuid=${$.activityData.actorInfo.uuid}&couponId=${$.oneCouponInfo.couponInfo.couponId}`;
			break;
		case 'startDraw':
			url = `https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/${type}`;
...
function getPostRequest(url, body) {
	let headers = {
		...
		'Referer': 'https://lzkjdz-isv.isvjcloud.com/wxFansInterActionActivity/activity/' + $.activityID +
			'?activityId=' + $.activityID + '&shareuserid4minipg=jd_4806fb66e0f3e&shopid=undefined',
		'user-agent': $.UA,
...
async function getUA() {
	$.UA =
		`jdapp;iPhone;10.0.10;14.3;${randomString(40)};network/wifi;model/iPhone12,1;addressid/3364463029;appBuild/167764;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1`
}
