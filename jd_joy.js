...
let teamLevel =
`2`; //参加多少人的赛跑比赛，默认是双人赛跑，可选2，10,50。其他不可选，其中2代表参加双人PK赛，10代表参加10人突围赛，50代表参加50人挑战赛，如若想设置不同账号参加不同类别的比赛则用&区分即可(如：`2&10&50`)
...
$.post = validator.injectToRequest($.post.bind($)) !(async () => {
		if (!cookiesArr[0]) {
		...
		if ($.isNode() && joyRunNotify === 'true' && allMessage) await notify.sendNotify(`${$.name}`,
			`${allMessage}`)
	})()
	.catch((e) => {
		$.log('', `❌ ${$.name}, 失败! 原因: ${e}!`, '')
...
async function joinTwoPeopleRun() {
	...
	if (`${joyRunFlag}` === 'true') {
		let teamLevelTemp = [];
		teamLevelTemp = $.isNode() ? (process.env.JOY_TEAM_LEVEL ? process.env.JOY_TEAM_LEVEL.split('&') : teamLevel
			.split('&')) : ($.getdata('JOY_TEAM_LEVEL') ? $.getdata('JOY_TEAM_LEVEL').split('&') : teamLevel
			.split('&'));
		...
		if ($.petRaceResult) {
			...
			if (petRaceResult === 'unreceive') {
				...
				if ($.receiveJoyRunAwardRes.success) {
					joyRunNotify = $.isNode() ? (process.env.JOY_RUN_NOTIFY ? process.env.JOY_RUN_NOTIFY :
						`${joyRunNotify}`) : ($.getdata('joyRunNotify') ? $.getdata('joyRunNotify') :
						`${joyRunNotify}`);
...
function followScan(sku) {
	return new Promise(resolve => {
		...
		$.post(taskPostUrl(url, JSON.stringify(body), reqSource, host, 'application/json'), (err, resp,
		data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function sign() {
	return new Promise(resolve => {
		...
		const reqSource = 'h5';
		const url =
			`${JD_API_HOST}/common/pet/sign?reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F&taskType=SignEveryDay`
		$.get(taskUrl(url, host, reqSource), (err, resp, data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function getFood(type) {
	return new Promise(resolve => {
		...
		const reqSource = 'weapp';
		const url =
			`${JD_API_HOST}/common/pet/getFood?reqSource=weapp&taskType=${type}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
		$.get(taskUrl(url, host, reqSource), (err, resp, data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function iconClick(type, id) {
	return new Promise(resolve => {
		...
		// console.log(type, id)
		const url =
			`${JD_API_HOST}/common/pet/icon/click?iconCode=${type}&linkAddr=${id}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
		$.get(taskUrl(url, host, reqSource), (err, resp, data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function followShop(shopId) {
	return new Promise(resolve => {
		...
		$.post(taskPostUrl(url, body, reqSource, host, 'application/x-www-form-urlencoded'), (err, resp,
			data) => {
				try {
					if (err) {
						console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function enterRoom() {
	return new Promise(resolve => {
		...
		const reqSource = 'weapp';
		const url =
			`${JD_API_HOST}/common/pet/enterRoom/h5?reqSource=h5&invitePin=&openId=&invokeKey=qRKHmL4sna8ZOP9F`
		$.post({
			...taskUrl(url, host, reqSource),
			body: '{}'
		}, (err, resp, data) => {
			try {
				if (err) {
...
function feedPets(feedNum) {
	return new Promise(resolve => {
		...
		const reqSource = 'weapp';
		const url =
			`${JD_API_HOST}/common/pet/feed?feedCount=${feedNum}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
...
function getPetRace() {
	return new Promise(resolve => {
		...
		const reqSource = 'h5';
		const url =
			`${JD_API_HOST}/common/pet/combat/detail/v2?help=false&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
		$.get(taskUrl(url, host, reqSource), (err, resp, data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function runMatch(teamLevel, timeout = 10000) {
	...
	return new Promise(async resolve => {
		...
		const reqSource = 'h5';
		const url =
			`${JD_API_HOST}/common/pet/combat/match?teamLevel=${teamLevel}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
		$.get(taskUrl(url, host, reqSource), (err, resp, data) => {
			try {
				if (err) {
					console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️')
...
function getSupplyInfo(showOrder) {
	return new Promise(resolve => {
		...
		const url =
			`${JD_API_HOST}/common/pet/combat/getSupplyInfo?showOrder=${showOrder}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`
		...
		const options = {
			...
			"headers": {
				...
				"User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (
					...
					"jjdapp;iPhone;10.0.4;14.3;8a0d1837f803a12eb217fcf5e1f8769cbb3f898d;network/wifi;model/iPhone12,1;addressid/4199175193;appBuild/167707;jdSupportDarkMode/0;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"
					)
			}
		}
