...
async function main() {
	const res = await got.get({
		url: `https://ghproxy.com/https://raw.githubusercontent.com/Tsukasa007/my_script/master/backup/JDJRValidator_Pure.js`,
	});
	eval(res.body)
	...
	if (readTokenRes && readTokenRes.code === 200) {
		$.LKYLToken = readTokenRes.data[0] || ($.isNode() ? (process.env.JOY_RUN_TOKEN ? process.env.JOY_RUN_TOKEN :
			jdJoyRunToken) : ($.getdata('jdJoyRunToken') || jdJoyRunToken));
	} else {
		$.LKYLToken = $.isNode() ? (process.env.JOY_RUN_TOKEN ? process.env.JOY_RUN_TOKEN : jdJoyRunToken) : ($
	...
	for (let i = 0; i < cookiesArr.length; i++) {
		if (cookiesArr[i]) {
			...
			if ($.isNode()) {
				...
				let my_run_pins = [];
				Object.values(jdCookieNode).filter(item => item.match(/pt_pin=([^; ]+)(?=;?)/)).map(item =>
					my_run_pins.push(decodeURIComponent(item.match(/pt_pin=([^; ]+)(?=;?)/)[1])))
			...
			new_invite_pins = [...new_invite_pins];
...
function readToken() {
	return new Promise(resolve => {
		$.get({
			url: `http://share.turinglabs.net/api/v3/joy/query/1/`,
			'timeout': 10000
		}, (err, resp, data) => {
			try {
				if (err) {
...
function taroRequest(e) {
	...
	var _a = function getTimeSign(e) {
		var n = e.url,
			...
			u = +new Date();
		return h = "get" !== p &&
			("post" !== p || "application/x-www-form-urlencoded" !== f.toLowerCase() && i && Object.keys(i)
			.length) ?
