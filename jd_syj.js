					...
					if (safeGet(data)) {
						...
						if (data.code === 0) {
							...
							if (data.data) {
								...
								message +=
									`【第${alreadySignDays}日签到】成功，获得${todayPrize.beanAmount}京豆 🐶\n`;
								if (alreadySignDays === 7) alreadySignDays = 0;
								message +=
									`【明日签到】可获得${eachDayPrize[alreadySignDays].beanAmount}京豆 🐶\n`;
					...
					if (safeGet(data)) {
						...
						if (data['success']) {
							if (data['data'] && data['data']['floorInfoList']) {
								const floorInfo = data['data']['floorInfoList'].filter(vo => !!vo &&
									vo['code'] === "SWAT_RED_PACKET_ACT_INFO")[0];
								if (floorInfo.hasOwnProperty('token') && floorInfo['floorData']
									.hasOwnProperty('userActivityInfo')) {
									...
									if (activityExistFlag) {
										if (!redPacketOpenFlag) {
											...
											if (redPacketRewardTakeFlag) {
												console.log(
													`【做任务 天天领京豆】 ${beanAmountTakeMinLimit}京豆已领取`
													);
											} else {
												if (currActivityBeanAmount >=
													beanAmountTakeMinLimit) {
													//领取200京豆
													console.log(
														`【做任务 天天领京豆】 累计到${beanAmountTakeMinLimit}京豆可领取到京东账户\n【做任务 天天领京豆】当前进度：${currActivityBeanAmount}/${beanAmountTakeMinLimit}`
														)
			...
			try {
				if (err) {
					...
					if (safeGet(data)) {
						data = JSON.parse(data);
						if (data['success']) {
							$.taskData = data['data'].filter(vo => !!vo && vo['taskDataStatus'] !==
								3);
			...
			try {
				if (err) {
					...
					if (safeGet(data)) {
						...
						if (data['success']) {
							console.log(
								`活动开启成功，初始：${data.data && data.data['activityBeanInitAmount']}京豆`)
							$.vvipFlag = true;
						} else {
...
async function distributeBeanActivity() {
	try {
		...
		if (!$.tuan && ($.assistStatus === 3 || $.assistStatus === 2 || $.assistStatus === 0) && $
			.canStartNewAssist) {
			console.log(`准备再次开团`)
			await openTuan()
				...
				if (err) {
					...
					if (safeGet(data)) {
						...
						if (data['success']) {
							...
							} else if (data.data.assistStatus === 3 && data.data
								.canStartNewAssist) {
...
async function getRandomCode() {
	await $.http.get({
		url: `https://raw.fastgit.org/zero205/updateTeam/master/shareCodes/jd_zz.json`,
		timeout: 10000
	}).then(async (resp) => {
		if (resp.statusCode === 200) {
			try {
