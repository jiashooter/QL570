...
[task_local]
#来客有礼
08 10 * * * https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_sendBeans.js, tag=来客有礼, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
=================================Loon===================================
[Script]
cron "08 10 * * *" script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_sendBeans.js,tag=来客有礼
===================================Surge================================
来客有礼 = type=cron,cronexp="08 10 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_sendBeans.js
====================================小火箭=============================
来客有礼 = type=cron,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_sendBeans.js, cronexpr="08 10 * * *", timeout=3600, enable=true
 */
const $ = new Env('来客有礼');
...
if ($.isNode()) {
	...
	let openCount = Math.floor((Number(cookiesArr.length)) / Number($.completeNumbers));
...
async function invite() {
	const url =
		`https://draw.jdfcloud.com/common/api/bean/activity/invite?openId=oPcgJ4_X7uCMeTgGmar-rmiWst1Y&activityId=${$.activityId}&userSource=mp&formId=123&jdChannelId=&fp=&appId=wxccb5c536b0ecd1bf&invokeKey=qRKHmL4sna8ZOP9F`;
	const method = `POST`;
	const headers = {
		'content-type': `application/json`,
		'Connection': `keep-alive`,
...
async function getActivityDetail() {
	const url =
		`https://draw.jdfcloud.com/common/api/bean/activity/detail?activityId=${$.activityId}&userOpenId=oPcgJ4_X7uCMeTgGmar-rmiWst1Y&timestap=${Date.now()}&userSource=mp&jdChannelId=&appId=wxccb5c536b0ecd1bf&invokeKey=qRKHmL4sna8ZOP9F`;
	const method = `GET`;
	const headers = {
		'cookie': $.cookie,
		'openId': `oPcgJ4_X7uCMeTgGmar-rmiWst1Y`,
