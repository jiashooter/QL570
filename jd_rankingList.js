				...
				try {
					...
					for (let i in data.result.taskList) {
						...
						if (data.result.taskList[i].taskItemInfo.status === 0) {
							await doTrumpTask(data.result.taskList[i].taskId, data.result
								.taskList[i].taskItemInfo.itemId, 1000)
						} else {
							console.log('已完成')
						}
					}
...
function msgShow() {
	...
	for (let i in merge) {
		...
		if (merge[i].notify.split("").reverse()[0] === "\n") merge[i].notify = merge[i].notify.substr(0, merge[i].notify
			.length - 1);
		message += `${merge[i].prizeDesc.split(STRSPLIT)[0]}${merge[i].prizeDesc.split(STRSPLIT)[1]}：` + (merge[i]
			.success ? `${merge[i].prizeCount.toFixed(merge[i].fixed)}${merge[i].prizeDesc.split(STRSPLIT)[2]}\n` :
			`失败：${merge[i].notify}\n`)
	}
	//合计
	if (needSum) {
		...
		for (let i in merge) {
			if (typeof(merge[i]) !== "object" || !merge[i].show) continue;
			if (typeof($.sum[merge[i].prizeDesc.split(STRSPLIT)[1]]) === "undefined") $.sum[merge[i].prizeDesc.split(
				STRSPLIT)[1]] = {
				count: 0
			};
...
function Env(t, e) {
	...
	return new class {
		...
		time(t, g = -8) {
			...
			let d = new Date().getTime() + f * 60 * 1000 - (g * 60 * 60 * 1000);
			let n = new Date(d);
			let e = {
				"M+": n.getMonth() + 1,
				"d+": n.getDate(),
