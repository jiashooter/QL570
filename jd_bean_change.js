...
================Loon===============
[Script]
cron "2 20 * * *" script-path=jd_bean_change.js, tag=京东资产变动通知
=============Surge===========
[Script]
		...
		if (response && response.code === "0") {
			...
			if (detailList && detailList.length > 0) {
				for (let item of detailList) {
					...
					if (new Date(date).getTime() >= tm1 && (!item['eventMassage'].includes("退还") && !item[
							'eventMassage'].includes('扣赠'))) {
						todayArr.push(item);
					} else if (tm <= new Date(date).getTime() && new Date(date).getTime() < tm1 && (!item[
							'eventMassage'].includes("退还") && !item['eventMassage'].includes('扣赠'))) {
						//昨日的
					...
					if (data) {
						...
						if (data.ret === 0) {
							data['expirejingdou'].map(item => {
								console.log(
									`${timeFormat(item['time'] * 1000)}日过期京豆：${item['expireamount']}\n`
									);
							})
			...
			try {
				if (err) {
					...
					if (data) {
						...
						$.jxRed = 0, $.jsRed = 0, $.jdRed = 0, $.jdhRed = 0, $.jxRedExpire = 0, $
							.jsRedExpire = 0, $.jdRedExpire = 0, $.jdhRedExpire = 0;
						let t = new Date()
						...
						$.expiredBalance = ($.jxRedExpire + $.jsRedExpire + $.jdRedExpire).toFixed(
							2)
						$.message +=
...
function timeFormat(time) {
	...
	return date.getFullYear() + '-' + ((date.getMonth() + 1) >= 10 ? (date.getMonth() + 1) : '0' + (date.getMonth() +
