...
!(async () => {
	...
	if (allMessage) {
		...
		if ($.isNode() && nowTimes.getDate() === 1 && (process.env.JDZZ_NOTIFY_CONTROL ? process.env
				.JDZZ_NOTIFY_CONTROL === 'false' : !!1)) {
			...
			try {
				if (err) {
					...
					if (safeGet(data)) {
						...
						if (flag && $.taskList.filter(item => !!item && item['taskId'] === 3) && $
					...
					if (safeGet(data)) {
						...
						if (func === "doInteractTask") {
							if (data.subCode === "S000") {
								console.log(
									`任务完成，获得 ${data.data.taskDetailResList[0].incomeAmountConf} 金币，${data.data.taskDetailResList[0].beanNum} 京豆`
									)
...
function readShareCode() {
	...
	return new Promise(async resolve => {
		$.get({
			url: `https://code.chiang.fun/api/v1/jd/jdzz/read/${randomCount}/`,
			'timeout': 10000
		}, (err, resp, data) => {
			try {
				if (err) {
