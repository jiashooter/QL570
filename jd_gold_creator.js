		...
		if (cookiesArr[i]) {
			...
			if (!$.isLogin) {
				...
				if ($.isNode()) {
					...
					$.setdata('',
					`CookieJD${i ? i + 1 : ""}`); //cookie失效，故清空cookie。$.setdata('', `CookieJD${i ? i + 1 : "" }`);//cookie失效，故清空cookie。
				}
				continue
			}
			await main()
				...
				if (err) {
					...
					if (safeGet(data)) {
						...
						if (data.code === '0') {
							...
							await goldCreatorDetail($.subTitleInfos[0]['matGrpId'], $.subTitleInfos[
									0]['subTitleId'], $.subTitleInfos[0]['taskId'], $
								.subTitleInfos[0]['batchId'], true);
							$.subTitleInfos = $.subTitleInfos.filter(vo => !!vo && vo[
								'hasVoted'] === '0');
					...
					if (safeGet(data)) {
						...
						if (data.code === '0') {
							if (data.result.taskCode === '0') {
								...
								if (data.result.lotteryScore) $.beans += parseInt(data.result
									.lotteryScore);
							} else {
