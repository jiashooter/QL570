...
$.UA = $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($
	.getdata('JDUA') ? $.getdata('JDUA') :
				...
				if (err) {
					...
					if (data) {
						...
						if (data.success && data.data) {
							...
							signFlag = true;
							console.log(
								`${turnTableId[i].name} 签到成功:获得 ${Number(data.jdBeanQuantity)}京豆`)
						} else {
							if (data.errorMessage) {
								if (data.errorMessage.indexOf('已签到') > -1 || data.errorMessage.indexOf(
										'今天已经签到') > -1) {
									signFlag = true;
								}
				...
				if (err) {
					...
					if (data) {
						...
						if (data.success && data.data) {
							...
							if (!data.hasSign) {
								...
								if (data.records && data.records[0]) {
									for (let i in data.records) {
										...
										if ((item.hasSign == false && item.index != 1) || i == data
											.records.length - 1) {
							...
							if (data.errorMessage) {
								if (data.errorMessage.indexOf('已签到') > -1 || data.errorMessage
									.indexOf('今天已经签到') > -1) {
									signFlag = true;
								}
								console.log(`${turnTableId[i].name} ${data.errorMessage}`)
...
function tasPostkUrl(turnTableId) {
	const url =
		`${JD_API_HOST}/api/turncard/channel/sign?turnTableId=${turnTableId}&fp=${fp}&eid=${eid}&invokeKey=qRKHmL4sna8ZOP9F`
	return {
		url,
		headers: {
			"Accept": "application/json, text/plain, */*",
