					...
					if (safeGet(data)) {
						...
						if (data.status) {
							...
							for (let i = 0; i < missions.length; ++i) {
								...
								if (mission['missionStatus'] === 0 && (mission['missionType'] ===
										1 || mission['missionType'] === 5)) {
...
function doMission(missionId) {
	return new Promise(resolve => {
		$.post(taskPostUrl('v1/game/mission', {
			"missionId": missionId
		}), async (err, resp, data) => {
			try {
				if (err) {
...
function receiveMission(missionId) {
	return new Promise(resolve => {
		$.post(taskPostUrl('v1/user/mission/receive', {
			"missionId": missionId
		}), async (err, resp, data) => {
			try {
				if (err) {
					...
					if (safeGet(data)) {
						...
						if (data.status) {
							if (data.data.remainPoint >= data.data.oncePoint) {
								...
								console.log(
									`当前赛点：${data.data.remainPoint}/${data.data.oncePoint}无法兑换京豆`
									)
								message +=
									`当前赛点：${data.data.remainPoint}/${data.data.oncePoint}，无法兑换京豆\n`
							}
