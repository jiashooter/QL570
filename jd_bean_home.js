					...
					if (safeGet(data)) {
						...
						if (data.data.jklInfo) {
							...
							if (!shareCode) {
								...
								// 去做逛会场任务
								if (data.data.beanActivityVisitVenue && data.data
									.beanActivityVisitVenue.taskStatus === '0') {
		...
		$.post(taskUrl('findBeanHome', body), async (err, resp, data) => {
			try {
				if (err) {
					...
					if (safeGet(data)) {
						data = JSON.parse(data);
						let beanTask = data.data.floorList.filter(vo => vo.floorName ===
							"种豆得豆定制化场景")[0]
						if (!beanTask.viewed) {
							await receiveTask()
							await $.wait(3000)
						...
						let tasks = data.data.floorList.filter(vo => vo.floorName === "赚京豆")[0][
							'stageList'
						]
