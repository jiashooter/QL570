/*
全民运动会守卫红包
0 11 * * * https://raw.githubusercontent.com/wql70255/QL570/main/jd_olympicgames_guard.js
更新时间：2021-7-10
活动入口：首页右侧下方
...
let cookiesArr = []
let cookie = ''
let inviteId = process.env.olympicgames_inviteId ?? ""//填写助力码
let uuid = randomString(40);

!(async () => {
    if(inviteId=="")console.log("请填写助力码")
    await requireConfig()
    if (!cookiesArr[0]) {
...
function requestApi(functionId, params) {
    ...
    return new Promise(resolve => {
        console.log(`functionId=olympicgames_${functionId}&body=${JSON.stringify(params)}&client=wh5&clientVersion=1.0.0&uuid=${uuid}&appid=o2_act`)
        $.post({
