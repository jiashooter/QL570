...
function getPlayTaskCenter() {
	...
	return new Promise(resolve => {
		$.post(taskPostClientActionUrl(`body={"client":"app"}&client=wh5&clientVersion=1.0.0`,
			`playTaskCenter`), async (err, resp, data) => {
				try {
					if (err) {
...
function doPlayAction(playId) {
	//await $.wait(20)
	return new Promise(resolve => {
		$.post(taskPostClientActionUrl(
			`body={"client":"app","playId":"${playId}","type":"1"}&client=wh5&clientVersion=1.0.0`,
			`playAction`), async (err, resp, data) => {
			try {
				if (err) {
					console.log(`${JSON.stringify(err)}`)
					console.log(`${$.name} API请求失败，请检查网路重试`)
